/* ui_iron_condor.js
 *
 * Front-end controller for the Iron Condor strategy page.
 * ------------------------------------------------------------------
 * – Builds dynamic stock inputs and dual-range sliders
 * – Sends filter payload to /api/fetch-iron-condor
 * – Renders results via ResultsUI
 * – Lets user tweak option price (0 = Bid … 100 = Ask)
 *   without extra API calls, affecting BOTH put and call sides simultaneously.
 */
...
(() => {
     ...
     * Local state
     * ----------------------------------------------------------------- */
    const LS_SAVED_STOCKS = "savedStocksIronCondor";
    ...
    /* -----------------------------------------------------------------
     * Market-price slider - adjusts BOTH put and call sides simultaneously
     * ----------------------------------------------------------------- */
    function attachSliders() {
      ...
      wrap.innerHTML = `
        ...
        <div class="slider-container">
          <div class="slider-group">
            <div class="slider-label">Option Pricing Strategy (Both Put & Call Sides)</div>
          ...
          <div class="pricing-details">
            <div class="pricing-section">
              <h4>PUT Side</h4>
              <span>BUY Lower Put: <span id="buyPutPct" style="font-weight: bold;">50%</span></span><br>
              <span>SELL Higher Put: <span id="sellPutPct" style="font-weight: bold;">50%</span></span>
            </div>
            <div class="pricing-section">
              <h4>CALL Side</h4>
              <span>SELL Lower Call: <span id="sellCallPct" style="font-weight: bold;">50%</span></span><br>
              <span>BUY Higher Call: <span id="buyCallPct" style="font-weight: bold;">50%</span></span>
            </div>
          </div>
        </div>`;
      ...
      const recalc = () => {
        ...
        // Convert slider value (0-50) to percentages
        // When slider = 50 (max): all at 50% (mid/mid)
        // When slider = 0 (min): buy at 100%, sell at 0% (conservative)
        const buyPct = 100 - sliderValue;  // For buying options (we pay)
        const sellPct = sliderValue;       // For selling options (we receive)

        // Update displays
        buyPutPctDisplay.textContent = `${buyPct}%`;
        sellPutPctDisplay.textContent = `${sellPct}%`;
        sellCallPctDisplay.textContent = `${sellPct}%`;
        buyCallPctDisplay.textContent = `${buyPct}%`;
        ...
        const recalculated = rawRecords.map(r => {
          // PUT side - credit spread (sell higher, buy lower)
          const putLowerPrice = r.putLowerBid + (buyPct / 100) * (r.putLowerAsk - r.putLowerBid);
          const putHigherPrice = r.putHigherBid + (sellPct / 100) * (r.putHigherAsk - r.putHigherBid);

          // CALL side - credit spread (sell lower, buy higher)
          const callLowerPrice = r.callLowerBid + (sellPct / 100) * (r.callLowerAsk - r.callLowerBid);
          const callHigherPrice = r.callHigherBid + (buyPct / 100) * (r.callHigherAsk - r.callHigherBid);
          ...
          const maxRisk = (r.spreadWidth * 100) - totalCredit;
          const pctGain = maxRisk > 0 ? (totalCredit / maxRisk) * 100 : 0;
          const annPctGain = r.dte > 0 ? (pctGain * 365) / r.dte : 0;

          return { ...r, putCredit, callCredit, totalCredit, maxRisk, pctGain, annPctGain };
        }).filter(x => x.totalCredit > 0);

        resultsUI.setRecords(recalculated);
        resultsUI.render();

        document.getElementById("message").textContent =
          `Found ${recalculated.length} profitable iron condors (re-calculated).`;
      };
    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      document.getElementById("tradeForm").addEventListener("submit", async e => {
        ...
        msg.textContent = `Loading iron condor options for ${symbols.length} stock(s)…`;
        resultsUI.setRecords([]);
        resultsUI.render();

        try {
          const res = await fetch("/api/fetch-iron-condor", {
            method: "POST",
          ...
          if (rawRecords.length === 0) {
            msg.textContent = "No profitable iron condors found.";
            return;
          }

          msg.textContent =
            `Found ${rawRecords.length} profitable iron condors. API calls: ${data.apiCalls}`;

          attachSliders();          // show price slider
