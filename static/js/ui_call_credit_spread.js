/* ui_call_credit_spread.js
 *
 * Front-end controller for the OTM Call Credit Spread strategy page.
 * ------------------------------------------------------------------
 * - Builds dynamic stock inputs and dual-range sliders
 * - Sends filter payload to /api/fetch-call-credit-spread
...
(() => {
     ...
     * Local state
     * ----------------------------------------------------------------- */
    const LS_SAVED_STOCKS = "savedStocksCallCreditSpread";
    ...
    function initializeRangeSliders() {
      // Strike range for OTM calls: 100-200% (above current price)
      initSlider("strike", {
        min: 100, max: 200, startMin: 125, startMax: 180,
        suffix: "%", minInputId: "minStrike", maxInputId: "maxStrike"
      });
      initSlider("dte", {
        min: 1, max: 800, startMin: 30, startMax: 100,
    ...
    function createStockSection() {
      ...
      const renderSaved = () => {
        ...
        savedStocks.forEach(s => {
          ...
          wrap.innerHTML = `
            ...
            <button type="button" class="remove-saved-btn" data-stock="${s}">x</button>`;
      ...
      const addInputRow = () => {
        ...
        row.innerHTML = `
          ...
          <button type="button" class="remove-input-btn">x</button>`;
    ...
    /* -----------------------------------------------------------------
     * Market-price slider with modern styling
     * ----------------------------------------------------------------- */
    function attachSliders() {
        ...
        <div class="slider-container">
          <div class="slider-group">
            ...
            <div class="slider-track">
              <span class="slider-bounds">Conservative<br><small>Sell Low / Buy High</small></span>
              <input type="range" id="unifiedSlider" min="0" max="50" value="50" class="modern-slider">
              <span class="slider-bounds">Aggressive<br><small>Sell High / Buy Low</small></span>
      ...
      const recalc = () => {
        ...
        // For call credit spread: SELL lower strike (we want higher price), BUY higher strike (we want lower price)
        // When slider = 50 (max): sell = 50%, buy = 50% (mid/mid)
        // When slider = 0 (min): sell = 0%, buy = 100% (conservative - sell at bid, buy at ask)
        ...
        const recalculated = rawRecords.map(r => {
          // Lower strike (SELL CALL) - we receive this price
          const lowerPrice = r.lowerBid + (sellCallPct / 100) * (r.lowerAsk - r.lowerBid);
          // Higher strike (BUY CALL) - we pay this price
          const higherPrice = r.higherBid + (buyCallPct / 100) * (r.higherAsk - r.higherBid);
        ...
        document.getElementById("message").textContent =
          `Found ${recalculated.length} profitable OTM call credit spreads (re-calculated).`;
      };
    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      document.getElementById("tradeForm").addEventListener("submit", async e => {
        ...
        try {
          const res = await fetch("/api/fetch-call-credit-spread", {
            method: "POST",
          ...
          if (rawRecords.length === 0) {
            msg.textContent = "No profitable OTM call credit spreads found.";
            return;
          }

          msg.textContent =
            `Found ${rawRecords.length} profitable OTM call credit spreads. API calls: ${data.apiCalls}`;
