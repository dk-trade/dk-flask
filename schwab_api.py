...
* OTM Call Credit Spread→ ``fetch_call_credit_spread_data()``
...
class SchwabAPI:
    ...
    # ------------------------------------------------------------------#
    # NEW – OTM Call Credit Spread strategy                             #
    # ------------------------------------------------------------------#

    def fetch_call_credit_spread_data(
        self,
        *,
        symbols: List[str],
        min_strike_pct: int,
        ...
        For each symbol, finds vertical call credit spreads where both options
        are OTM (out of the money - above current price) and the spread is profitable.
        ...
        for sym in symbols:
            try:
                recs, calls = self._fetch_stock_call_credit_spread(
                    symbol=sym,
                    min_strike_pct=min_strike_pct,
                    max_strike_pct=max_strike_pct,
                    min_dte=min_dte,
    ...
    def _fetch_stock_call_credit_spread(
        self,
        *,
        symbol: str,
        min_strike_pct: int,
        ...
        # Strike boundaries - percentages of current price for OTM CALL strikes
        # Both strikes should be ABOVE current price (OTM for calls)
        # min_strike_pct=125 means 125% of current price (25% OTM)
        # max_strike_pct=180 means 180% of current price (80% OTM)
        min_strike = price * (min_strike_pct / 100)
        max_strike = price * (max_strike_pct / 100)
        ...
        # Iterate over expirations
        for exp_key, strike_map in call_map.items():
            # Get all strikes for this expiration and sort them ascending
            strikes = [float(s) for s in strike_map.keys()]
            strikes.sort()

            # Filter strikes within our OTM range (above current price)
            valid_strikes = [s for s in strikes if min_strike <= s <= max_strike]

            if len(valid_strikes) < 2:
                continue  # Need at least 2 strikes for a spread

            # For each lower strike (the one we SELL - closer to ATM, more expensive)
            for i, lower_strike in enumerate(valid_strikes):
                lower_strike_str = self._find_strike_key(strike_map, lower_strike)
                if not lower_strike_str:
                    continue
                ...
                # For each higher strike within max_spread distance (the one we BUY)
                for higher_strike in valid_strikes[i+1:]:
                    spread_width = higher_strike - lower_strike
                    if spread_width > max_spread:
                        continue  # Spread too wide

                    higher_strike_str = self._find_strike_key(strike_map, higher_strike)
                    if not higher_strike_str:
                        continue
                    ...
                    # For call credit spread: sell lower (more expensive), buy higher (cheaper)
                    # Credit = lower_mid - higher_mid (should be positive)
                    if lower_mid <= higher_mid:
                        continue  # No credit available

                    # Calculate spread metrics
                    credit_received = (lower_mid - higher_mid) * 100  # Net credit received
                    max_risk = (spread_width * 100) - credit_received  # Max loss

                    if credit_received <= 0:
                        continue  # Only profitable spreads

                    # For call credit spreads, max gain is the credit received
                    max_gain = credit_received
                    pct_gain = (max_gain / max_risk) * 100 if max_risk > 0 else 0
                    ann_pct_gain = (pct_gain * 365) / dte if dte > 0 else 0

                    # Calculate strike percentage (lower strike as percentage of current price)
                    strike_pct = (lower_strike / price) * 100
